...
public class TelaUsoLook extends JFrame {
    ...
    public TelaUsoLook(Usuario usuario, Look look) {
    	setTitle("Uso look");
        ...
        usarBtn.addActionListener(e -> {
        	int i = 0;
        	int j = 0;
        	for(Item item : look.verPecasLook().values()) {
        	...
        	if(i==0) {
        		if(j>0) {
        			new TelaUsarLook(usuario,look).setVisible(true);
        			dispose();
        		}else {
        			JOptionPane.showMessageDialog(null,"O look não contém nenhum item para ser usado.","Atenção",JOptionPane.WARNING_MESSAGE);
        		}
        	}else {
    			JOptionPane.showMessageDialog(null,"O look contém item(ns) que esta(ão) emprestado(s), então não pode ser usado.","Atenção",JOptionPane.WARNING_MESSAGE);
    		}
        });
        ...
        for (Map.Entry<LocalDateTime, String> entry : look.verUsosLook().entrySet()) {
