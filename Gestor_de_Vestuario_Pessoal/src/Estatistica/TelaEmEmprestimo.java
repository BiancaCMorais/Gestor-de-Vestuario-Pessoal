...
public class TelaEmEmprestimo extends JFrame{
    ...
    public TelaEmEmprestimo(Usuario usuario) {
    	setTitle("Itens em emprestimo");
        ...
        JLabel titulo = new JLabel("Itens em emprestimo ("+usuario.verNomeUsuario()+")");
        ...
        List<String> emprestados = estatisticas.emprestados(usuario);
        int linhaAtual=1;
        String texto;
        if(!usuario.verVestuario().isEmpty() && !emprestados.isEmpty()) {
        	for (String emprestado : emprestados) {
                JLabel lavagem = new JLabel(emprestado);
